...
async function insertProperty(PropertyData, usersTableRes) {
  ...
  console.log("Formatted properties with lat/lng:", formattedProperty);

  await db.query(
    format(
      "INSERT INTO properties (host_id, name, location, property_type, price_per_night, description, latitude, longitude) VALUES %L",
      formattedProperty
    )
  );
}
